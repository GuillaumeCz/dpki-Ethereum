const Dpki = artifacts.require('./Dpki.sol');

contract('Dpki', (accounts) => {
	it('...should associate a name to an ipfs address', () => {
		const key = "ssh-rsa ";
    const userName = 'Bob';

		let dpkiInstance;
		
		return Dpki.deployed().then(instance => {
				dpkiInstance = instance;
				return dpkiInstance.registerKeyAddress(userName, key);
			}).then(() => dpkiInstance.getKeyAddressFromName(userName)).then(res => {
          assert.equal(res, key, 'the addresses are fucked up bro... ! ')
			});
	})
})
