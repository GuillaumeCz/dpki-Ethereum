const Dpki = artifacts.require('./Dpki.sol');

contract('Dpki', (accounts) => {
	it('...should link an address to a key', () => {
		const currentUserAddress = accounts[0];
		const key = "ssh-rsa ";
		let dpkiInstance;
		
		return Dpki.deployed().then(instance => {
				dpkiInstance = instance;
				return dpkiInstance.registerKey(key);
			}).then(() => Promise.all(
				[ dpkiInstance.getKey(currentUserAddress), 
					dpkiInstance.getAddress(key) ]
			)).then(res => {
				assert.equal(res[0], key, 'The key is not what was expected');
				assert.equal(res[1], currentUserAddress, 'The address is not what was expected');
			});
	})
})
